...
const SerializedProductsList = ({ searchTerm = '', branch = '', sortField = 'name', sortOrder = 'asc' }) => {
  ...
  const fetchItems = async () => {
    try {
      ...
      const response = await fetch(`${SummaryApi.getInventoryByType.url}/serialized-product${branchQuery}`, {
  ...
  const sortByStock = (a, b) => {
    const stockA = a.stock ? a.stock.length : 0;
    const stockB = b.stock ? b.stock.length : 0;
        ...
        {selectedStockItem && (
          <div>
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-500">
                Current Available Stock: {selectedStockItem.stock ? selectedStockItem.stock.length : 0} {selectedStockItem.unit}
              </p>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {stockHistory.map((historyItem, index) => (
                      <tr key={historyItem.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-medium">{historyItem.serialNumber}</td>
