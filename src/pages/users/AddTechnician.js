...
const AddTechnician = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(SummaryApi.addTechnicianUser.url, {
        method: SummaryApi.addTechnicianUser.method,
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.success) {
        // Redirect to technician users list on success
        navigate('/users/technicians');
      } else {
        setError(data.message || 'Failed to add technician user');
      }
    } catch (err) {
      setError('Server error. Please try again later.');
      console.error('Error adding technician user:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <div className="flex items-center mb-6">
        ...
        <h1 className="text-2xl font-semibold text-gray-800">Add New Technician</h1>
      </div>
